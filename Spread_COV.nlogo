...
to setup
  ...
  ask n-of initial-infected turtles [get-sick]
  reset-ticks
end
...
to lock-down
 ...
 if strict? = False and medium? = False and weak? = False [
    set transmission-rate infectivity]
  if strict? = False and (count turtles with [infected?] / count turtles) > when [
    set transmission-rate lockdown-transmission * 0.04
  set strict? True]
  if strict? and (count turtles with [infected?] / count turtles) < release-pop [
    set strict? False ]
 if medium? [
    set transmission-rate lockdown-transmission / 3 ]
 if weak? [
    set transmission-rate lockdown-transmission / 2 ]
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
